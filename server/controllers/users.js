...
module.exports = {
     ...
     * 获取用户已购书籍
     */
    getBoughtActivitys: function(req, res, next) {

        Users.getBoughtActivitys(req.query.skey)
        .then(function(resData){
            if(resData && resData.length) {
                res.json({
                    result: 0,
                    list: resData.filter(function(item){
                        return item.bkid && item.bkclass && item.bkname && item.bkauthor && item.bkpublisher && item.bkfile && item.bkcover && item.bkprice
                   })
                });
            } else {
                res.json({
